...
export const PropertyBodyValues = ({ values }: {values: PropertyProps['values']}) => {
    return (
        ...
        values.map((item, key) => {
            ...
            return (
                <div className={`flex flex-col sm:flex-row w-full h-auto py-3 sm:py-0 min-h-16 ${isFirstValue} ${borders}`} 
                    key={item?.id}>
                    <div className="sm:flex-1 max-w-48 flex flex-col sm:flex-row px-4 sm:px-0 sm:items-center justify-center">
                        <span className="font-outfit text-base font-bold text-secondary sm:text-center">{item.name}</span>   
                    </div>
